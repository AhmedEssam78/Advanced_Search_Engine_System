...
public:
    ...
    WebPage()
    {
        name = ""; impression = 0; clicked = 0; CTR = 0.0; PR = 0.0; score = 0.0; PRtrial = 0.0;
        keywords.resize(0);
        ToWeb.resize(0);
        FromWeb.resize(0);
    }
    ...
    void set_Impression(int n) {
        impression = n;
    }
    void set_clicked(int n) {
        clicked = n;
    }
    void set_PR(double n) {
        PR = n;
    }
    void set_PRtrial(double n) {
        PRtrial = n;
    }
    void increment_click() {
        clicked++;
    }
    void increment_impression() {
        impression++;
    }
    void set_CTR() {
        CTR = (double(clicked) / double(impression)) * 100.0;
    }
    void set_score() {
...
map <string, vector <string>> read_keywords()
{
    ...
    fstream fin;
    fin.open("Keyword file.csv", ios::in);
    vector<string> row;
...
void update_impression(string name, int impression)
{
    fstream fin, fout;
    fin.open("Number of impressions file.csv", ios::in);
    fout.open("Number of impressions file_new.csv", ios::out);
    int count = 0, i;
    string roll1;
    string line, word;
    vector<string> row;
    ...
    rename("Number of impressions file_new.csv", "Number of impressions file.csv");
}
void update_clicked(string name, int clicked, vector <WebPage>& arr)
{
    fstream fin, fout;
    fin.open("clicked.csv", ios::in);
    fout.open("clicked_new.csv", ios::out);
    int count = 0, i;
    string roll1;
    string line, word;
    vector<string> row;
    ...
    remove("clicked.csv");
    rename("clicked_new.csv", "clicked.csv");
}
...
vector <WebPage*> search_query(string str, vector <WebPage>& web)
{
    ...
    for (int i = 0; i < strings.size(); i++)
    {
        for (int j = 0; j < web.size(); j++)
        {
            if (AND == "")
            {
                if (find(web[j].keywords.begin(), web[j].keywords.end(), strings[i]) != web[j].keywords.end())
                {
                    if (visited[j] == false)
            ...
            {
                if ((find(web[j].keywords.begin(), web[j].keywords.end(), strings[i]) != web[j].keywords.end()) && (find(web[j].keywords.begin(), web[j].keywords.end(), AND) != web[j].keywords.end()))
                {
                    if (visited[j] == false)
                        matched.push_back(&web[j]);
                    visited[j] = true;
...
void searchFace(vector <WebPage>& arr)
{
    ...
    if (matched.size() == 0)
    {
        ...
        searchFace(arr); return;
    }
    else 
    {
        matched = sort_result(matched);
        resultFace(matched, arr); return;
    }
}
void resultFace(vector <WebPage*>& matched, vector <WebPage>& arr)
{
    ...
    for (int i = 1; i <= matched.size(); i++)
    {
        ...
        update_impression(matched[i-1]->get_name(), matched[i-1]->get_impression());
        cout << i << ". " << matched[i - 1]->get_name() << /* " --> " << matched[i-1]-> get_score() << */ endl;
    }
    cout << endl;
...
void viewFace(vector <WebPage*>& matched, vector <WebPage>& arr)
{
    ...
    update_clicked(matched[num3-1]->get_name(), matched[num3-1]->get_clicked(), arr);
    cout << endl;
...
int main()
{
    ...
    //{
    //    cout << webpages[i].get_name() << endl;
    //    cout << "keywords: ";
    //    for (int j = 0; j < webpages[i].keywords.size(); j++)
    //        cout << webpages[i].keywords[j] << ", ";
    //    cout << endl;
    //    cout << "Looking to: ";
    //    for (int j = 0; j < webpages[i].ToWeb.size(); j++)
    //        cout << webpages[i].ToWeb[j]->get_name() << ", ";
    //    cout << endl;
    //    cout << "And from: ";
    //    for (int j = 0; j < webpages[i].FromWeb.size(); j++)
    //        cout << webpages[i].FromWeb[j]->get_name() << ", ";
    //    cout << endl;
    //    cout << "PR: " << webpages[i].get_PR() << endl;
    //    cout << "Clicked: " << webpages[i].get_clicked() << endl;
    //    cout << "Impression: " << webpages[i].get_impression() << endl;
    //    webpages[i].set_CTR();
    //    cout << "CTR: " << webpages[i].get_CTR() << endl;
    //    webpages[i].set_score();
    //    cout << "score: " << webpages[i].get_score() << endl;
    //    cout << endl;
    //    cout << endl;
    //}
    vector <WebPage> webpages = readingFiles();
